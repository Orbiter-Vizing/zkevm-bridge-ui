...
import { useDefaultBridgeWithdrawFormStyles } from "src/views/home/components/default-bridge-withdraw-form/default-bridge-withdraw-form.styles";
...
interface DefaultBridgeWithdrawFormProps {
  // account: string;
  formData?: FormData;
  onResetForm: () => void;
...
export const DefaultBridgeWithdrawForm: FC<DefaultBridgeWithdrawFormProps> = ({
  ...
  const classes = useDefaultBridgeWithdrawFormStyles();
  ...
  const getL2EstimatedGas = useCallback(async () => {
    ...
    if (
      ...
      const omniGasLimit = getOmniChainGasLimit(vizingChain.chainId, bridgeChain.chainId);
